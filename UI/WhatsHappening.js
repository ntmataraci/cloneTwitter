...
const WhatsHappening = ({ myMsg }) => {
  const [boxTextScrollHeight, setBoxTextScrollHeight] = useState();
  const expandBox = (e) => {
    setBoxTextScrollHeight(e.target.scrollHeight);
    boxTextScrollHeight < e.target.scrollHeight
      ? (e.target.style.height = e.target.scrollHeight + "px")
      : "";
    counterText(e.target.value.length);
    e.target.value.length>0?setNullMsg(false):setNullMsg(true)
  };

  const [textCountRounder, SetTextCounterRounder] = useState("55");
  const counterText = (e) => {
    SetTextCounterRounder(() => {
      return 55 + e;
    });
  };
...
const [nullMsg,setNullMsg]=useState(true)
  const getMsg = () => {
    let ms = Date.now();
    myMsg({msg:inputEl.current.value,time:ms});
    inputEl.current.value=""
    setNullMsg(true)
  };

useEffect(()=>{
textCountRounder>120&&setNullMsg(true)
},[textCountRounder])
  ...
  return (
    <div>
      ...
      <div className={style.everyoneReplyButton__container}>
        <svg viewBox="0 0 24 24" aria-hidden="true" className={style.worldIcon}>
          <g>
        ...
        <button className={style.everyoneReplyButton}>
          Everyone can reply
        </button>
      </div>
      <div
        style={{
          ...
          borderBottom: "0.5px solid rgba(128, 128, 128, 0.521)",
        }}
      >
        <div className={style.otherIcons__container}>
          <svg
            viewBox="0 0 24 24"
            aria-hidden="true"
            className={style.otherIcons}
          >
            <g>
        ...
        >
          {textCountRounder < 100 ? (
            ...
            >
              ...
              <circle
                ...
                stroke="#1D9BF0"
                style={{
                  strokeDashoffset: "43.6233",
                  strokeDasharray: textCountRounder,
                }}
              ></circle>
            </svg>
          ) : (
            <div
              className={style.counterHelper}
              style={
                textCountRounder > 120
                  ?{ border: "1px red solid" }
                  : { border: "1px green solid" }
              }
            >
              {120 - textCountRounder}{" "}
            </div>
          )}
          <svg
            viewBox="0 0 24 24"
            aria-hidden="true"
            className={style.otherIcons}
            style={{ borderRadius: "50%", border: "0.5px solid gray" }}
          >
            <g>
          ...
          {nullMsg ?
              <button disabled className={style.tweetButton+" "+style.tweetButton__null} >
              Tweet
            </button>
            :
            <button className={style.tweetButton} onClick={getMsg}>
            Tweet
          </button>
          }
